...
;;; Commentary:
;; An offline client for Micro.blog.
...
(defun microblog-init-db ()
  ...
  (let ((db (sqlite-open microblog-db-file)))
    ...
    (condition-case nil
        (sqlite-execute db "ALTER TABLE posts ADD COLUMN is_draft INTEGER DEFAULT 0")
      (error nil))
...
(defun microblog-submit-update (&optional as-draft)
  "Parse the buffer and send to Micro.blog. Handles both New and Edited posts.
If AS-DRAFT is non-nil, the post is sent to Micro.blog as a draft."
  (interactive)
  (let ((title "") (categories "") (body "")
    ...
    (save-excursion
      ...
      (let* ((cat-list (if (string-empty-p categories) '() (split-string categories "," t "[ \t]+")))
             (temp-file (make-temp-file "microbe-payload"))
             (payload nil))
        ...
        (if is-new
            (let ((props `((content . ,(vector body)))))
              ...
              (when as-draft (push `(post-status . ["draft"]) props))
          ...
          (let ((replace-alist `((content . ,(vector body)))))
            ...
            ;; If updating a draft to publish, explicitly set published status
            (if as-draft 
                (push `(post-status . ["draft"]) replace-alist)
              (push `(post-status . ["published"]) replace-alist))
            (setq payload `((action . "update") (url . ,post-url) (replace . ,replace-alist)))))
        
        (with-temp-file temp-file (insert (json-encode payload)))
        (message (if as-draft "Saving draft to Micro.blog..." (if is-new "Publishing new post..." "Sending update...")))
        
        ;; Send via curl for robust networking (using -i to get headers)
          ...
          (with-current-buffer curl-output-buffer
            ...
            (let ((http-status nil)
              ...
              (when (re-search-forward "^[Ll]ocation:[ \t]*\\(.*\\)\r?$" nil t)
                (setq location-url (string-trim (match-string 1))))
              
              (if (member http-status '("200" "201" "202" "204"))
                  (progn
                    (kill-buffer edit-buf)
                    
                    ;; Persist the Draft locally using the returned Location URL
                    (let* ((final-url (or location-url post-url))
                           (final-id (or post-id final-url))
                           (db (sqlite-open microblog-db-file))
                           (draft-flag (if as-draft 1 0))
                           (date (format-time-string "%Y-%m-%dT%H:%M:%S%:z")))
                      
                      (sqlite-execute db "INSERT OR REPLACE INTO posts (id, date_published, content_html, url, title, categories, is_draft) VALUES (?, ?, ?, ?, ?, ?, ?)"
                                      (list final-id date body final-url title categories draft-flag)))
                    
                    (if (and is-new (not as-draft))
                        (progn
                          (message "Published! Fetching latest posts...")
                          (microblog-sync-posts))
                      (message (if as-draft "Success! Draft saved." "Success! Post updated.")))
                    
                    ;; Refresh UI if open
                    (let ((list-buf (get-buffer "*Micro.blog Headers*"))
                          (drafts-buf (get-buffer "*Micro.blog Drafts*")))
                      (when list-buf
                        (with-current-buffer list-buf
                          (setq tabulated-list-entries (microblog-get-entries))
                          (tabulated-list-print t)))
                      (when drafts-buf
                        (with-current-buffer drafts-buf
                          (setq tabulated-list-entries (microblog-get-draft-entries))
                          (tabulated-list-print t)))))
                (error "Operation failed! HTTP %s. Server said: %s" 
...
(defvar microblog-edit-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c C-z") 'microblog-save-draft)
...
(defun microblog-get-entries ()
  "Query DB and format rows for tabulated list (excluding drafts)."
  (let* ((db (sqlite-open microblog-db-file))
         (rows (condition-case nil
                   (sqlite-select db "SELECT id, date_published, content_html, url, title, categories FROM posts WHERE is_draft IS NULL OR is_draft = 0 ORDER BY date_published DESC")
                 (error (sqlite-select db "SELECT id, date_published, content_html, url, title, categories FROM posts ORDER BY date_published DESC")))))
    (mapcar (lambda (row)
              (let* ((id (nth 0 row))
                     (date (nth 1 row))
...
(defun microblog-get-draft-entries ()
  "Query DB and format rows exclusively for drafts."
  (let* ((db (sqlite-open microblog-db-file))
         (rows (condition-case nil
                   (sqlite-select db "SELECT id, date_published, content_html, url, title, categories FROM posts WHERE is_draft = 1 ORDER BY date_published DESC")
                 (error nil))))
    (mapcar (lambda (row)
              (let* ((id (nth 0 row))
                     (date (nth 1 row))
...
(defun microblog-save-draft ()
  "Save the current buffer as a draft to Micro.blog."
  (interactive)
  (microblog-submit-update t))

(defun microblog-drafts ()
  "Open the Micro.blog Drafts view."
  (interactive)
  (microblog-init-db)
  (let ((buf (get-buffer-create "*Micro.blog Drafts*")))
...
(define-key microblog-headers-mode-map (kbd "d") 'microblog-drafts)
